    ...
    <>
      <div className='my-16 flex flex-col px-4 text-center lg:my-32'>
        <div className='items-center pb-16'>
          <h2 className='pb-4 text-transparent bg-border-gradient bg-clip-text'>{heading}</h2>
        ...
        <div className='grid grid-cols-1 gap-x-4 gap-y-8 lg:grid-cols-4 '>
          {card.map((card, index) => (
            <div
              className={`rounded-3xl bg-border-gradient ${index % 2 !== 0 ? 'from-secondary-md1 to-primary-lt2' : 'from-primary-lt2 to-secondary-md1'}  p-[3px]`}
              key={index}
            >
              <div className='flex h-full flex-col items-center gap-4 rounded-[calc(1.5rem-3px)] bg-neutral-9 p-6'>
                <h3 className=''>{card.heading}</h3>
                <Icon icon={card.icon} className='text-8xl text-primary-lt1' />
