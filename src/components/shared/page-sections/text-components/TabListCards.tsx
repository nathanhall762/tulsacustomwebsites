...
const TabListCards: React.FC<Props> = ({
  ...
  return (
    <>
      <div>
        <div className='mx-auto my-16 flex flex-col items-center rounded-3xl bg-neutral-7 p-8 px-4 text-center shadow-xl lg:my-32 lg:p-16 lg:pl-32 lg:mx-12 lg:pb-24'>
          ...
          <div className='lg:flex w-full flex-col-reverse items-center lg:flex-row lg:justify-center'>
            ...
            <div className={`${!selected ? '' : 'hidden'} w-full min-h-3xl`}>
              <div
                style={{ backgroundImage: `url('${defaultCard.image}')` }}
                aria-label={defaultCard.alt_image_text}
                className='group flex w-full flex-grow flex-col h-3xl justify-end bg-cover bg-center shadow-2xl backdrop-blur-sm lg:m-0'
              >
            ...
            {/* card body */}
            {cards.map((card) => (
              <div className={`${selected === card.heading ? '' : 'hidden'} w-full`}>
                <div
